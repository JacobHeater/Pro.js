 ...
 *****************************************************************************
 *****************************************************************************/
define('pro.cryptography', ['pro', 'pro.collections', 'pro.serialization'], function (pro, collections, serialization) {
    "use strict";
    ...
    pro.module('cryptography', pro.object({
        ...
        exoCrypt: pro.object({
            ...
            exoCrypt: pro.$class('pro.cryptography.exoCrypt.exoCrypt', function (password, salt, channel) {
                ...
                var _serializer = serialization.string;
                    ...
                    if (exoCryptPackage && this.IsConnected && exoCryptPackage.IsConnected && exoCryptPackage.ProxyInstance && this.ProxyInstance && exoCryptPackage.ProxyInstance.ID == this.ProxyInstance.ID && this.ProxyInstance.IsValidConnection && exoCryptPackage.ProxyInstance.IsValidConnection) {
                        if (exoCryptPackage.Token === _hashedPassword && !exoCryptPackage.IsLocked) {
                            ...
                            if (pro.string.contains(realData, _nonPrimiToken)) {
                                ...
                                output = encoding.base64.decodeBase64(realData);
                ...
                this.Encrypt = function (obj) {
                    ...
                    if (pro.isString(obj)) {
                        bytes = serialization.string.getBytes(obj);
                        isPrimitive = false;
                    } else if (pro.isObject(obj)) {
                        bytes = serialization.object.getBytes(obj);
                        isPrimitive = false;
                    ...
                    var base64String = encoding.base64.encodeBase64(bytes);
            ...
            exoHasher: pro.singleton('pro.cryptography.exoHash.exoHasher', {
                    ...
                    if (arguments.length === 2) {
                        if (pro.isArray(arguments[0])) {
                            ...
                            if (incoming.length > 0) {
                                ...
                                var strIncoming = serialization.string.getString(incoming);
                                var scrambleIncoming = pro.string.scramble.calculated(strIncoming, 5);
                                filler = scrambleIncoming + salt + filler;
                                var fillerBytes = serialization.string.getBytes(filler);
                                var max = 255;
                                var multiple = 2;
                                var obfuscated = 3;
                                var _byte = 0;
                                ...
                                var hasher = function () {
                                    ...
                                    hash.length = 0;
                                    chars = serialization.string.getChars(bytes);
                                    bytes = serialization.string.getBytes(chars);
                                    if (bytes.length > nBytes) {
                                        bytes = collections.asEnumerable(bytes).take(nBytes).toArray();
                                        ...
                                        do {
                                            ...
                                            masFillerBytes = serialization.string.getBytes(masFiller);
                            ...
                            var bytes = serialization.string.getBytes(string);
                            ...
                            return hash;
                            var bytes = serialization.string.getBytes(json);
                            var hash = this.computeHash(bytes, arguments[1]);
                        } else {
                            return [];
                        }
                        ...
                        var key = arguments[0];
                        var keyBytes = serialization.string.getBytes(key);
                        var incoming = arguments[1];
                        if (!pro.isString(incoming)) {
                            incoming = pro.toJson(incoming);
                        }
                        var incomingBytes = serialization.string.getBytes(incoming);
                        var newBytes = keyBytes.concat(incomingBytes);
                        newBytes = collections.array.scramble.calculated(newBytes, 5);
                ...
                compare: function (hash1, hash2) {
                    if (pro.isArray(hash1) && pro.isArray(hash2)) {
                        var strHash1 = serialization.string.getString(hash1);
                        var strHash2 = serialization.string.getString(hash2);
