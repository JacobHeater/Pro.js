...
ever because of the utilization of the collections.JS library for quickly
 ...
 *****************************************************************************
 *****************************************************************************/
define('pro.document', ['pro', 'pro.collections'], function (pro, collections) {
    "use strict";
    //Private objects
    var regex = {
    ...
    var getHtmlArray = function (str) {
        var html = new collections.list();
        if (pro.isString(str)) {
            var matches = str.match(regex.html);
            matches = collections.asEnumerable(matches);
            if (matches.count() === 1) {
                str = str.replace(/[<]+/g, '\r\n<').replace(/[>]+/g, '>\r\n');
                matches = collections.asEnumerable(str.match(regex.html)).select(function (m) {
                    return m.trim();
                });
            }
            if (matches.count() > 0) {
                matches.forEach(function (i, m) {
                    var htmlArr = collections.asEnumerable(m.split(/[\r\n]+/)).select(function (s) {
                        return s.trim();
                    });
    ...
    var getStyleObj = function (str) {
        var obj = {};
        if (pro.isString(str)) {
            var styles = collections.asEnumerable(str.split(';')).where(function (s) {
                return s !== ""
            });
	...
	var readyHandlers = new collections.list();
    ...
    pro.module('document', pro.object({
        ...
        ready: function (readyHandler) {
            ...
            if (pro.isUndefined(document.onreadystatechange)) {
        ...
        queryResult: pro.$class('pro.document.queryResult', function (array, context) {
            array = collections.asEnumerable(array).distinct().toArray();
        ...
        query: function (selector, context) {
            ...
            if (ctxt) {
                ...
                if (ctxt.querySelectorAll) {
                    result = collections.array.merge(result, ctxt.querySelectorAll(selector));
                } else {
                    var processResult = function (selector) {
                        ...
                        switch (c0) {
                            case '#':
                                ...
                                result = collections.array.merge(result, right);
                                break;
                            case '.':
                                var right = ctxt !== document ? new pro.document.queryResult([ctxt], ctxt).children().where(function (e) {
                            ...
                            case '[':
                                if (selector.match(regex.selectors.attr).length > 0) {
                                    ...
                                    if (attrValue !== null) {
                                        ...
                                        result = collections.array.merge(result, new pro.document.queryResult(children).where(function (e) {
                            ...
                            default:
                                if (regex.selectors.name.test(c0) && regex.selectors.noSpecialChars.test(selector)) {
                                    ...
                                    result = collections.array.merge(result, document.getElementsByTagName(selector));
                                }
                                break;
                        }
                    };
                    if (selector.indexOf(',') > -1) {
                        var selectors = collections.asEnumerable(selector.split(',')).where(function (s) {
                            return s !== "";
                        }).select(function (s) {
                            return s.trim();
                        });
            ...
            if (pro.isString(contents)) {
                ...
                if (tagName.length > 0 && tagName !== "") {
                    ...
                    if (attributes.length > 0) {
                        attributes = collections.asEnumerable(attributes).select(function (kvp) {
                            var split = kvp.split('=');
                            var key = split[0];
        ...
        queryString: {
            ...
            keyValuePairs: function () {
                var location = window.location,
                  ...
                  enumerable = collections.asEnumerable(keyValuePairs);
    ...
    pro.extend(pro.object({
        ...
        AjaxRequest: pro.$class('pro.AjaxRequest', function (configuration) {
            ...
            this.headers = new collections.dictionary();
    ...
    pro.document.queryResult.extend(pro.object({
        on: function (event, handler) {
            ...
            if (pro.isString(event) && event.trim() !== "" && pro.isFunction(handler)) {
                ...
                this.forEach(function (i, e) {
                    if (!e.proEventCache || !(e.proEventCache && pro.isClass(e.proEventCache) && e.proEventCache.is(collections.dictionary))) {
                        e.proEventCache = new collections.dictionary();
                    }
        ...
        off: function (event, id) {
            ...
            if (pro.isString(event) && event !== "") {
                ...
                this.forEach(function (i, e) {
                    if (e.proEventCache && pro.isClass(e.proEventCache) && e.proEventCache.is(collections.dictionary)) {
        ...
        attr: function (key, value) {
            var $this = this;
            if (arguments.length === 1 && pro.isClass(key) && key.is(collections.dictionary)) {
        ...
        classList: function () {
            if (arguments.length === 0) {
                return collections.asEnumerable(this.first() !== undefined ? (this.first().className.split(' ').length > 0 ? this.first().className.split(' ') : []) : []).where(function (n) {
                    return n !== "" && n.length > 0;
                }).toList();
            } else if (arguments.length === 1 && pro.isArray(arguments[0]) || pro.isEnumerable(arguments[0])) {
                var c = collections.asEnumerable(arguments[0]);
                this.forEach(function (i, o) {
                    o.className = queryResult(o).classList().clear().addRange(c).distinct().join(' ');
                });
            } else if (arguments.length >= 1) {
                var c = collections.asEnumerable(Array.apply(this, arguments));
                this.forEach(function (i, o) {
                    o.className = queryResult(o).classList().clear().addRange(c).distinct().join(' ');
                });
            }
